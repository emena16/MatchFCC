...
abstract class ProfileBaseController extends Controller
{
    ...
    protected function initPrivacy(UserCoreModel $oUserModel, stdClass $oUser)
    {
        ...
        if (!$this->bUserAuth && $oPrivacyViewsUser->privacyProfile === 'only_members') {
            $this->view->error = t('Whoops! "%0%" El perfil solo es visivle para los miembros. Por Favor <a href="%1%">loguate </a> o <a href="%2%">registrate</a> para ver el perfil.',
            ...
            $this->view->error = t('Whoops! "%0%" El perfil no esta Disponible para ti.', $oUser->username);
        }
    ...
    protected function getMailLink($sFirstName, stdClass $oUser)
    {
        if ($this->bUserAuth) {
            ...
            $aUrlParms = [
                'msg' => t('Regístrese ahora para enviarle un mensaje %0%!', $sFirstName),
                'ref' => 'profile',
                'a' => 'mail',
                'u' => $oUser->username,
                'f_n' => $sFirstName,
    ...
    protected function getMessengerLink($sFirstName, stdClass $oUser)
    {
        if ($this->bUserAuth) {
            ...
            $aUrlParms = [
                'msg' => t('Registrese ahora para hablar con %0%.', $sFirstName),
                'ref' => 'profile',
                'a' => 'messenger',
                'u' => $oUser->username,
                'f_n' => $sFirstName,
    ...
    protected function getBeFriendLink($sFirstName, stdClass $oUser)
    {
        if ($this->bUserAuth) {
            ...
            $aUrlParms = [
                'msg' => t('Registrate gratis %site_name% para convertirte en amigo de %0%.', $sFirstName),
                'ref' => 'profile',
                'a' => 'befriend',
                'u' => $oUser->username,
                'f_n' => $sFirstName,
