<div class="row">
    ...
    {if !$browser->isMobile()}
        <div class="left col-xs-12 col-sm-4 col-md-3">
            ...
            <ul>
                <li>
                    <a href="{{ $design->url('user','setting','avatar') }}" title="{lang 'Change My Profile Photo'}"><i class="fa fa-upload"></i> {lang 'Cambiar Foto'}</a>
                </li>
                <li>
                    <a href="{{ $design->url('user','setting','edit') }}" title="{lang 'Edit My Profile'}"><i class="fa fa-cog fa-fw"></i> {lang 'Editar Perfil'}</a>
                </li>
                <li>
                    <a href="{{ $design->url('user','setting','design') }}" title="{lang 'My Wallpaper'}"><i class="fa fa-picture-o"></i> {lang 'Design Profile'}</a></li>
                <li>
                    <a href="{{ $design->url('user','setting','notification') }}" title="{lang 'My Email Notification Settings'}"><i class="fa fa-envelope-o"></i> {lang 'Notificaciones'}</a>
                </li>
                <li>
                    <a href="{{ $design->url('user','setting','privacy') }}" title="{lang 'My Privacy Settings'}"><i class="fa fa-user-secret"></i> {lang 'Configurar Privacidad'}</a>
                </li>
                <li>
                    <a href="{{ $design->url('payment','main','info') }}" title="{lang 'My Membership'}"><i class="fa fa-credit-card"></i> {lang 'Detalles de membresía'}</a>
                </li>
                <li>
                    <a href="{{ $design->url('user','setting','password') }}" title="{lang 'Change My Password'}"><i class="fa fa-key fa-fw"></i> {lang 'Cambiar '}</a>
                </li>
            </ul>
        </div>
    {/if}
    ...
    <div class="left col-xs-12 col-sm-6 col-md-6">
        <h2 class="center underline">{lang 'Ultimos Usuarios'}</h2>
        {{ $userDesignModel->profilesBlock() }}

        <h2 class="center underline">{lang 'Mis amigos'}</h2>
        ...
        <h2 class="center underline">{lang 'Visitantes que visitaron mi perfil'}</h2>
        ...
        {if $is_picture_enabled}
            <h2 class="center underline">{lang 'Mis álbumes de fotos'}</h2>
        ...
        {if $is_video_enabled}
            <h2 class="center underline">{lang 'Mis álbumes de Video'}</h2>
        ...
        {if $is_forum_enabled}
            <h2 class="center underline">{lang 'Mis Discuciones'}</h2>
        ...
        {if $is_note_enabled}
            <h2 class="center underline">{lang 'Mis notas'}</h2>
        ...
        <h2 class="center underline italic s_tMarg">{lang 'Búsqueda rápida de usuarios'}</h2>
        {{ SearchUserCoreForm::quick() }}
    </div>

    <div class="left col-xs-12 col-sm-2 col-md-3">
        <h2>{lang 'Ultimas Noticias'}</h2>
        <div id="wall"></div>
    </div>
</div>
